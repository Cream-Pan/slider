...
温熱環境主観評価 WebApp は，屋外温熱環境実験において，参加者の主観的な温熱評価，温熱的な快・不快方向への変化イベント，任意タイミングの温熱評価，スマートフォンの位置情報を同時に記録する Web アプリケーションである．

定期地点評価では，温冷感と温熱的快・不快の2項目を記録する．イベント入力は「無」，「快・不快方向」，「定期評価と同じ項目」から選択できる．「快・不快方向」では参加者が「快適な方向に変化した」または「不快な方向に変化した」と感じた場合に専用ボタンを押し，「定期評価と同じ項目」では同じ2項目を好きなタイミングで回答する．
...
本アプリは，屋外温熱環境実験において，参加者の主観評価，温熱的な快・不快方向への変化イベント，任意タイミングの温熱評価，位置情報を記録するための Web アプリケーションである．

実験開始前に参加者 ID とイベント入力の方法を選択し，実験中は以下の記録を扱う．
...
- **任意評価**
  - イベント入力で「定期評価と同じ項目」を選択した場合に利用できる．
  - 定期地点評価と同じ2項目を好きなタイミングで回答する．
...
| ファイル | 内容 |
|---|---|
| `index.html` | 画面構成，定期地点評価・任意評価モーダル，方向変化ボタン，操作ボタンを定義する． |
...
定期地点評価，任意評価，方向変化イベント，GPSデータ，実験セッション状態をブラウザ内へ逐次保存するために使用する．
...
実験中には，「方向変化イベント」，「任意評価」，「定期地点評価」の3種類を記録できる．イベント入力で「無」を選択した場合は，方向変化イベントと任意評価を記録しない．
...
イベント入力で「定期評価と同じ項目」を選択した場合，最初の定期地点評価を Submit した後から，評価領域の「任意評価」ボタンを使用できる．任意評価では定期地点評価と同じ温冷感・温熱的快不快の2項目を回答する．保存時の `trigger_type` は `event_evaluation` とし，定期地点評価の進行，現在区間の更新，実験終了判定には影響しない．
...
- 「快適な方向に変化した」ボタン
- 「不快な方向に変化した」ボタン
- 「任意評価」ボタン（イベント入力で「定期評価と同じ項目」を選択した場合）
- 定期地点評価ボタン
...
方向変化イベントではモーダルを表示せず，ボタンを押した時刻のみを即時に記録する．任意評価では定期地点評価と同じモーダルを表示し，2項目の回答を保存する．
...
定期地点評価，任意評価，方向変化イベントを同じCSVへ保存する．

| 列名 | 内容 |
|---|---|
| `trigger_type` | `checkpoint`，`comfortable_change`，`uncomfortable_change`，`event_evaluation` のいずれか |
| `segment_id` | 定期地点ID，またはイベント発生時の現在区間 |
| `evaluation_started_at` | 定期地点評価または任意評価を開いた時刻，または方向変化ボタンを押した時刻 |
| `evaluation_submitted_at` | 定期地点評価または任意評価を保存した時刻，または方向変化ボタンを押した時刻 |
| `response_duration_ms` | 定期地点評価・任意評価の回答時間 [ms]．方向変化イベントでは `0` |
...
event_evaluation,SUN1,2026/09/07 13:08:05.104,2026/09/07 13:08:10.812,5708,やや暖かい,快い
```
...
| `subjective` | 定期地点評価，任意評価，方向変化イベントのレコード |
| `gps` | GPSレコード |
...
開始前にイベント入力で「定期評価と同じ項目」を選択した場合は，実験中に「任意評価」を押すと温冷感と温熱的快・不快を回答できる．「完了して保存」を押すと，定期地点評価とは別の `event_evaluation` として保存される．
...
* 任意評価では定期地点評価と同じ2項目を回答し，`trigger_type=event_evaluation` として保存する．
